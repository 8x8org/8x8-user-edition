...
| 4 | `obra/superpowers` | Extract workflow patterns | Useful process discipline without importing a second execution authority. |
| 5 | `uber/ADR` | Prepare an isolated documentation POC | Potentially useful governance tooling with reversible Markdown output. |
...
| 8 | `cloudflare/computer` | Restricted sandbox research only | Computer use can cross browser, account, network and action boundaries. |
...
| 13 | `vercel/next.js` | Defer pending a product requirement | The current public client does not justify a full framework migration. |
...
3. Whether `pdf-inspector` should be invoked as a local binary, isolated service or ephemeral job.
4. Whether Supervision can deliver meaningful mobile value within measured CPU, memory and storage ceilings.
5. Whether a future tenant product warrants Next.js while the current public release remains intentionally static and auditable.
...
4. **ADR-001:** Test reversible ADR generation in a disposable branch.
...
7. **COMPUTER-001:** Design a fake-site-only browser canary with an explicit domain allowlist and zero credentials.
8. **AIRLLM-001:** Produce a hardware feasibility report before downloading any model.

## Completion boundary

MSG197 research is complete only when every candidate has an independent council receipt or a documented owner-approved deferral. Installation, deployment and production readiness remain separate candidate-level missions.
