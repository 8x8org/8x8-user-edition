(() => {
  ...
  function classify(candidate) {
    ...
    if (decision.includes('NARROW_SUBSET_ELIGIBLE') && packet.includes('CANARY_PASS')) {
      return { key: 'READY_FOR_ADAPTER_DESIGN', label: 'Adapter design eligible', className: 'ready' };
    }
    if (decision.includes('REJECT') || packet.includes('BLOCKED') || runtime.includes('BLOCKED')) {
      return { key: 'BLOCKED', label: 'Blocked', className: 'blocked' };
    }
    if (decision.includes('DEFER')) {
      return { key: 'DEFERRED', label: 'Deferred', className: 'deferred' };
    }
    if (decision.includes('PATTERN') || decision.includes('SCHEMA') || decision.includes('IDEAS') || decision.includes('KNOWLEDGE') || decision.includes('TAXONOMY')) {
      return { key: 'PATTERNS_ONLY', label: 'Patterns only', className: 'patterns' };
    }
  ...
  function candidateCard(candidate) {
    ...
    identity.append(
      textNode('h3', candidate.id),
      textNode('div', candidate.repository, 'repo')
    );
    header.append(identity, textNode('span', status.label, `status ${status.className}`));

    article.append(
      header,
      textNode('div', candidate.pin, 'pin'),
      textNode('div', candidate.decision, 'decision'),
      textNode('div', candidate.runtime, 'runtime')
    );
    ...
    if (candidate.evidence && typeof candidate.evidence === 'object') {
      ...
      for (const [key, value] of Object.entries(candidate.evidence)) {
        ...
        evidence.append(textNode('span', `${key}: ${printable}`));
      }
  ...
  function renderCandidates() {
    ...
    const candidates = state.ledger.candidates.filter((candidate) => {
      if (state.filter === 'ALL') return true;
      return classify(candidate).key === state.filter;
    });

    for (const candidate of candidates) grid.append(candidateCard(candidate));
    if (!candidates.length) grid.append(textNode('p', 'No candidates match this filter.', 'error'));
  }

  function describeDependency(record) {
    if (record.id === 'AIRLLM_CUDA_BENCHMARK') {
      return 'Requires a dedicated owner-approved NVIDIA CUDA node, an approved model license, storage and cost authority, measured performance evidence, and verified cleanup. It is explicitly rejected on Samsung Termux and the active Ubuntu PRoot.';
    }
    if (record.id === 'REAL_RESEARCH_COUNCIL') {
      return 'Requires four real identity-, lease-, input-digest-, output-digest- and receipt-bound votes. ChatGPT advisory output does not count toward quorum.';
    }
    return String(record.state || 'Evidence remains incomplete.');
  }

  function renderRemaining() {
    const grid = byId('remainingGrid');
    grid.replaceChildren();
    for (const dependency of state.ledger.remaining_dependencies || []) {
      const article = document.createElement('article');
      article.append(
        textNode('h3', dependency.id),
        textNode('div', dependency.state, 'pin'),
        textNode('p', describeDependency(dependency))
      );
      grid.append(article);
    }
  }
