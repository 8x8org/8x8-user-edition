...
# Intentionally independent from benchmark.FRONTIER_STATUS_REQUIRED_MARKERS.
# This duplication is a truth-integrity redundancy boundary: if the validator
# accidentally drops an inherited negative marker, this test must be able to
# disagree rather than importing the same omission. PR #164 demonstrated this
# exact failure class with DISTRIBUTED_SESSION_BACKEND_NOT_IMPLEMENTED.
INDEPENDENT_FRONTIER_MARKERS = (
...
class ExternalBenchmarkTests(unittest.TestCase):
    ...
    def test_frontier_is_not_promoted_by_declaration(self) -> None:
        ...
        for marker in INDEPENDENT_FRONTIER_MARKERS:
            self.assertIn(marker, status)

    def test_independent_frontier_marker_contract_is_unique(self) -> None:
        self.assertGreater(len(INDEPENDENT_FRONTIER_MARKERS), 30)
        self.assertEqual(
            len(INDEPENDENT_FRONTIER_MARKERS),
            len(set(INDEPENDENT_FRONTIER_MARKERS)),
        )
