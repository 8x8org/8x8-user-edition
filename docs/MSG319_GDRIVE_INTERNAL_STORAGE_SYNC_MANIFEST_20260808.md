# MSG319 — Google Drive → Samsung Internal Storage Sync Manifest

Date: 2026-08-08

## Verified Pass 4B baseline

Samsung Ubuntu PRoot preserved all 200 currently downloadable GitHub Actions artifacts with 0 failures and 521,652,000 bytes preserved.

- Receipt: `/root/8x8-github-local/receipts/20260808T103233Z-artifact-preservation`
...
Recent canonical Drive documents observed on 2026-08-08 include MSG319, the canonical program ledger, owner-absence continuity packet, MSG291B agent teaching packet, MSG289 full-convergence packet, master continuation index, Hermes start-here packet, and the 2026-08-08 Kimi research/build/evidence archive.

## Local synchronization policy

`ops/msg319_gdrive_internal_storage_snapshot.sh` performs an additive timestamped snapshot into the existing writable Android `8x8 OS` directory.

It intentionally does **not** use destructive mirroring:

- no Drive deletion;
- no local deletion;
- no force overwrite of the existing canonical Android estate;
...
- root-level 8x8/Hermes/MSG continuity material is included even if not filed under the Drive `8x8 OS` folder;
...
Run the exact script from `feature/msg319-convergence-control-v1`, then require terminal marker:

`MSG319_GDRIVE_INTERNAL_STORAGE_SYNC_COMPLETE`

The resulting receipt is expected under:

`/root/8x8-github-local/receipts/<timestamp>-gdrive-internal-storage-sync`
